...
####################Implementing the BatteryPolynomialFit class####################################
import pybamm
import numpy as np
...
class BatteryPolynomialFit:
    ...
    def __init__(self, battery_model, parameter_values):
        ...
        self.nominal_current = self.parameter_values['Current function [A]']
        ...
        # Remove the maxium voltage and minimum voltage events
        # Due to these events the simnulation is interepted and doesnot take place
    ...
    def calculate_coefficients(self):
        experiment = pybamm.Experiment([(
            f"Charge at {self.nominal_current}A until {self.Vmax}V",
            f"Discharge at {self.nominal_current}A until {self.Vmin}V",
            f"Charge at {self.nominal_current}A until {self.Vmin + ((self.Vmax - self.Vmin) / 2)}V"
        )])
